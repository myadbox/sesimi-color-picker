# sesimi-color-picker

Brand Compliance Checker/Color Picker

| Table of Contents                 |
| --------------------------------- |
| [How to set up](#how-to-set-up)   |
| [How to upgrade](#how-to-upgrade) |

# How to set up

Navigate to:

https://github.com/asad310397/sesimi-color-picker

Navigate to Code > Download ZIP:

![image.png](./assets/readme-imgs/image.png)

Then open chrome

Type in: chrome://extensions

![image.png](./assets/readme-imgs/ext-url.png)

Then turn on developer mode:

![image.png](./assets/readme-imgs/dev-mode.png)

Then click on Load unpacked:

![image.png](./assets/readme-imgs/load.png)

Finally select the sesimi-color-picker and hit “Select” and you should have it loaded

![image.png](./assets/readme-imgs/open-folder.png)

# How to upgrade

Delete the sesimi-color-picker folder from where you saved it. Then repeat the set-up instructions again
